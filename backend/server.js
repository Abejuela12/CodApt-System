...
app.put('/api/auth/profile/:userId', async (req, res) => {
  try {
    ...
    if (req.body.email) { fields.push(`email = $${idx++}`); values.push(normEmail(req.body.email)); }
...
app.get('/api/admin/users', async (req, res) => {
  try {
    ...
    const enrichedUsers = await Promise.all(users.map(async (user) => {
      ...
      const TOTAL_POSSIBLE_CONCEPTS = 24; 
    ...
    const { rows: metricsRows } = await db.query(
      `WITH latest_scores AS (
       ...
       SELECT
         ...
         (SELECT COALESCE(ROUND(AVG(pct)), 0) FROM mastery_pct) AS avg_progress`
    );

...
app.post('/api/submit', async (req, res) => {
  try {
    ...
    //      insert the current submission. Checking this AFTER the insert
    //      was a real bug: the just-inserted row would count against
    //      itself, so `alreadySolved` was always true whenever the current
    //      submission was correct — meaning `newlySolved` was always 0 on
    //      every submission after the very first one for a concept. That
    //      permanently capped tasks_completed at 1 for any concept with
    //      more than one task, making mastery mathematically unreachable
    //      even when success_rate was 100%. Checking BEFORE the insert
    //      fixes this while still correctly preventing double-counting a
    //      task the learner had already solved in an earlier session.
    // SUPABASE (pg): { rows } and $1–$2
    ...
    // Fetch real total task count for the concept, SCOPED TO THE TIER being
    // played. Without the problem_tier filter here, a concept with tasks
    // across Beginner/Intermediate/Advanced would sum ALL of them into
    // total_tasks — making tasks_completed >= total_tasks unreachable for a
    // learner who only ever plays a single tier. That was the root cause of
    // "mastered on this device, still 0% everywhere else."
    ...
    // ── 9. Cosine similarity recommendation ──────────────────────
...
/* ══════════════════════════════════════
   API — GET ADMIN USERS
   (per-user mastery block lives INSIDE the map below, where `user` exists)
══════════════════════════════════════ */
